...
function x=meridian_arc_s(B)
	a0=1+3/4*e2+45/64*e2^2+175/256*e2^3;
	a1=3/8*e2+15/32*e2^2+512/1024*e2^3;
	a2=15/256*e2^2+105/1024*e2^3;
	a3=35/3072*e2^3;
	x=a*(1-e2)*(a0*B-a1*sin(2*B)+a2*sin(4*B)+a3*sin(6*B))
endfunction
x=meridian_arc_s(%pi/4)
x_q=integrate('(1-e2*sin(B)^2)^-1.5','B',0,%pi/4)*a*(1-e2)   /// integrate('function','variable',low bound,high bound)